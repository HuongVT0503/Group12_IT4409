...
export default function CommentItem({ 
  ...
  const isCommentAuthor = user?.id === item.author.id || user?.id === item.author.userId;
  const isPostAuthor = user?.id === postAuthorId;

  const canDelete = isCommentAuthor || isPostAuthor;

  const safeDate = (date) => {
      try { return formatDistanceToNow(new Date(date)) + " ago"; } 
      catch { return ""; }
  };
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 group">
          <div className="bg-gray-50 rounded-2xl rounded-tl-none px-4 py-2 inline-block max-w-full">
            ...
            <p className="text-sm text-gray-800 mt-1 whitespace-pre-wrap">{item.comment.content}</p>
          </div>
