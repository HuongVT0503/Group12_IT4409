...
export default function PostManagement() {
  ...
  const handleDeletePost = async (report) => {
    ...
    try {
      ...
      // Remove all reports related to this target from UI
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm">Review user reports and take action.</p>
        </div>
      </div>
        ...
        <table className="w-full text-left text-sm text-gray-600">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr><td colSpan="5" className="px-6 py-4 text-center text-gray-400">No pending reports.</td></tr>
            ) : reports.map((report) => (
              <tr key={report.reportId} className="hover:bg-gray-50/80">
                <td className="px-6 py-4">
                  ...
                  <div className="text-xs text-gray-400 mt-1">ID: {report.targetId}</div>
                </td>
                <td className="px-6 py-4 font-medium text-gray-900">{report.fromUser}</td>
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${
                    report.targetType === 'Post' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'
                  }`}>
                    {report.targetType}
                  </span>
                </td>
                <td className="px-6 py-4">
                  {report.createdAt ? formatDistanceToNow(new Date(report.createdAt)) + ' ago' : 'Unknown'}
                </td>
